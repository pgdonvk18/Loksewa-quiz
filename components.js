...
// १. युजरको कोइन अपडेट गर्ने र ऐतिहासिक रेकर्ड राख्ने फंक्सन
...
// २. ग्लोबल कम्पोनेन्टहरू (च्याट, फुटर र इमोजि) स्वतः लोड गर्ने IIFE
(function() {
  ...
  style.innerHTML = `
    ...
    .footer-col p, .footer-col a { color: #94a3b8; text-decoration: none; line-height: 1.6; display: block; margin-bottom: 6px; }
    ...
    <footer class="global-site-footer">
      <div class="footer-content">
        <div class="footer-col">
          <h4>LoksewaQuest</h4>
          <p>लोकसेवा आयोग तथा शिक्षक सेवा आयोगका परीक्षार्थीहरूका लागि तयार पारिएको प्लेटफर्म।</p>
        </div>
      </div>
...
// ३. ग्लोबल कन्फिगरेसन र रुम कार्ड फंक्सन
...
window.loadCreateRoomCard = function(containerId) {
  ...
  if (!document.getElementById('room-component-style')) {
    ...
    style.innerHTML = `
      ...
      .select-grid { display: grid; gap: 8px; grid-template-columns: repeat(3, 1fr); }
      .option-btn { padding: 10px; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 10px; font-size: 13px; font-weight: 600; color: #475569; cursor: pointer; text-align: center; }
      .option-btn.active { background: #eff6ff; border-color: #2563eb; color: #2563eb; }
      .btn { width: 100%; padding: 12px; border: none; border-radius: 10px; font-size: 14px; font-weight: 700; cursor: pointer; }
...
// ४. नेभिगेसन बार लोड गर्ने फंक्सन
window.loadBottomNav = function(activePage) {
  if (!document.getElementById('bottom-nav-style')) {
    ...
    styleElem.innerHTML = `
      ...
      .nav-link { display: flex; flex-direction: column; align-items: center; text-decoration: none; color: #94a3b8; font-size: 10px; font-weight: 600; gap: 3px; flex: 1; }
...
// ५. ग्लोबल पपअप र मोडलहरू (DOM Loaded भएपछि)
...
window.showSuccessPopup = function(message, redirectUrl, type = 'success', titleText = '') {
  ...
  if (titleEl) titleEl.textContent = titleText || 'सफल भयो! 🎉';
  if (iconBox) iconBox.className = `cg-icon-box ${type}`;
