// १. Firebase का модуलहरू सधैं फाइलको सुरुमा (Top) राख्नुपर्छ
...
// २. युजरको कोइन अपडेट गर्ने र History सेभ गर्ने फंक्सन
...
// ३. Global Components (Chat, Footer, Floating Emojis)
(function() {
  ...
  style.innerHTML = `
    ...
    .global-site-footer { background: #0f2a4a; color: #94a3b8; padding: 30px 20px 20px 20px; font-size: 13px; border-top: 1px solid #1e293b; margin-top: auto; width: 100%; }
    .footer-content { max-width: 1100px; margin: 0 auto; display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 25px; margin-bottom: 25px; }
    .footer-col h4 { color: #ffffff; font-size: 15px; font-weight: 700; margin-bottom: 12px; }
    .footer-col p, .footer-col a { color: #94a3b8; text-decoration: none; line-height: 1.6; display: block; margin-bottom: 6px; transition: color 0.2s; }
    .footer-col a:hover { color: #60a5fa; }
    .footer-socials { display: flex; gap: 12px; margin-top: 12px; }
    .footer-socials a { background: rgba(255,255,255,0.08); width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 50%; color: white; font-size: 14px; }
    .footer-socials a:hover { background: #2563eb; }
    .footer-bottom { max-width: 1100px; margin: 0 auto; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.1); display: flex; flex-direction: column; align-items: center; text-align: center; gap: 8px; font-size: 12px; }
  ...
  if (closeChatBtn) closeChatBtn.addEventListener('click', window.toggleGlobalChat);
  ...
  window.sendGlobalQuickEmoji = function(emoji) {
    ...
    if (window.sendChatMessageToDB) window.sendChatMessageToDB(emoji);
  };
  ...
  if (chatForm) {
    chatForm.addEventListener('submit', (e) => {
      ...
      if (!text) return;
      input.value = '';
      if (/^\p{Emoji}+$/u.test(text)) window.triggerGlobalFloatingEmoji(text);
      if (window.sendChatMessageToDB) window.sendChatMessageToDB(text);
    });
  }
})();

// ४. ग्लोबल कन्फिगरेसन र रुम कार्ड फंक्सनहरूलाई `window` मा जोड्ने
...
window.loadCreateRoomCard = function(containerId) {
  ...
  if (!document.getElementById('room-component-style')) {
    ...
    style.innerHTML = `
      ...
      .option-btn { padding: 10px; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 10px; font-size: 13px; font-weight: 600; color: #475569; cursor: pointer; text-align: center; transition: all 0.2s ease; }
      .option-btn.active { background: #eff6ff; border-color: #2563eb; color: #2563eb; box-shadow: 0 2px 5px rgba(37, 99, 235, 0.1); }
      .btn { width: 100%; padding: 12px; border: none; border-radius: 10px; font-size: 14px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; }
...
window.loadBottomNav = function(activePage) {
  if (!document.getElementById('bottom-nav-style')) {
    ...
    styleElem.innerHTML = `
      .bottom-nav { position: fixed; bottom: 0; left: 0; width: 100%; background: #ffffff; display: flex; justify-content: space-around; align-items: center; padding: 8px 4px 14px 4px; box-shadow: 0 -4px 20px rgba(0,0,0,0.08); border-radius: 20px 20px 0 0; z-index: 1000; }
      .nav-link { display: flex; flex-direction: column; align-items: center; text-decoration: none; color: #94a3b8; font-size: 10px; font-weight: 600; gap: 3px; flex: 1; transition: color 0.2s ease; }
    ...
    const modalHTML = `
      <div id="customGlobalModal" class="cg-modal-overlay">
        <div class="cg-modal-card">
          <div id="cgModalIconBox" class="cg-icon-box success"><i id="cgModalIcon" class="fa-solid fa-check"></i></div>
      ...
      <div id="customConfirmModal" class="cg-modal-overlay">
        <div class="cg-modal-card">
          <div class="cg-icon-box warning"><i class="fa-solid fa-triangle-exclamation"></i></div>
      ...
      <style>
        .cg-modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.65); backdrop-filter: blur(6px); z-index: 99999; display: flex; align-items: center; justify-content: center; opacity: 0; visibility: hidden; transition: all 0.25s ease-in-out; }
        .cg-modal-overlay.active { opacity: 1; visibility: visible; }
        .cg-modal-card { background: #ffffff; width: 90%; max-width: 360px; border-radius: 24px; padding: 26px 20px; text-align: center; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); border: 1px solid #f1f5f9; }
        .cg-icon-box { width: 58px; height: 58px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; margin: 0 auto 16px auto; }
        ...
        .cg-btn { flex: 1; background: #2563eb; color: white; border: none; padding: 12px; border-radius: 12px; font-weight: 700; cursor: pointer; }
...
window.showSuccessPopup = function(message, redirectUrl, type = 'success', titleText = '') {
  ...
  if (type === 'success' && !titleText) titleText = 'सफल भयो! 🎉';
  if (type === 'error' && !titleText) titleText = 'त्रुटि! ❌';
  if (type === 'warning' && !titleText) titleText = 'सचेत गराइएको छ! ⚠️';
